...
import { Droplets, Scissors, Shovel, Sprout, Bug, Leaf, X } from 'lucide-react';
...
function PlotCard({ plot, onAction, onPlant }) {
  ...
  const hasIssues = plot.hasPest || plot.hasWeeds;

  return (
    <div className={`rounded-lg border border-stone-700 p-4 ${stateInfo.bg} ${hasIssues ? 'ring-1 ring-red-500/50' : ''}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {plot.hasWeeds && (
            <span className="text-xs font-bold px-2 py-0.5 rounded text-lime-400 bg-lime-900/40 flex items-center gap-1">
              <Leaf size={10} /> 杂草
            </span>
          )}
      ...
      <div className="space-y-2 mb-4 text-xs text-stone-400">
        ...
        {(plot.state === FIELD_STATE.PLANTED || plot.state === FIELD_STATE.GROWING || plot.state === FIELD_STATE.READY) && (
          <>
            <div className="flex justify-between">
              <span className={plot.waterLevel < 60 ? 'text-orange-400' : ''}>
                水分{plot.waterLevel < 60 ? ' (减产)' : ''}
              </span>
              <div className="flex items-center gap-1.5">
                <div className="w-16 h-1.5 bg-stone-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: plot.waterLevel < 30 ? '#ef4444' : plot.waterLevel < 60 ? '#f59e0b' : '#3b82f6',
                    }}
                  />
                </div>
                <span className={plot.waterLevel < 30 ? 'text-red-400' : plot.waterLevel < 60 ? 'text-orange-400' : ''}>
                  {Math.floor(plot.waterLevel)}
                </span>
              </div>
            </div>
            <div className="flex justify-between">
        ...
        {plot.hasPest && (
          <div className="flex justify-between items-center">
            <span className="text-red-400">虫害严重度</span>
            <div className="flex items-center gap-1.5">
              <div className="w-16 h-1.5 bg-stone-700 rounded-full overflow-hidden">
                <div className="h-full bg-red-500 rounded-full transition-all"
                  style={{ width: `${plot.pestSeverity}%` }} />
              </div>
              <span className="text-red-400">{Math.floor(plot.pestSeverity)}%</span>
            </div>
          </div>
        )}

        {/* 杂草生长（未出现时显示积累进度） */}
        {!plot.hasWeeds && plot.weedGrowth > 0 && (plot.state === FIELD_STATE.GROWING || plot.state === FIELD_STATE.PLANTED) && (
          <div className="flex justify-between items-center">
            <span className="text-lime-600">杂草萌芽</span>
            <div className="flex items-center gap-1.5">
              <div className="w-16 h-1.5 bg-stone-700 rounded-full overflow-hidden">
                <div className="h-full bg-lime-700 rounded-full transition-all"
                  style={{ width: `${plot.weedGrowth}%` }} />
              </div>
              <span className="text-lime-600">{Math.floor(plot.weedGrowth)}%</span>
            </div>
          </div>
        )}
        ...
        {plot.hasWeeds && (
          <div className="flex justify-between items-center">
            <span className="text-lime-400">杂草茂盛</span>
            <div className="flex items-center gap-1.5">
              <div className="w-16 h-1.5 bg-stone-700 rounded-full overflow-hidden">
                <div className="h-full bg-lime-500 rounded-full transition-all"
                  style={{ width: `${Math.min(100, (plot.weedLevel / 150) * 100)}%` }} />
              </div>
              <span className="text-lime-400">{Math.floor(plot.weedLevel)}</span>
            </div>
          </div>
        )}

        {/* 减产提示 */}
        {yieldMod < 0.95 && (plot.state === FIELD_STATE.GROWING || plot.state === FIELD_STATE.READY) && (
          <div className="text-xs text-orange-400 mt-1">
            当前减产 {Math.round((1 - yieldMod) * 100)}%
          </div>
        )}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {plot.hasPest && (
          ...
          >
            <Bug size={12} /> 除虫 ({plot.pestClicks})
          </button>
        )}
        ...
        {plot.hasWeeds && (
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-lime-800/60 hover:bg-lime-700/60 text-lime-200 rounded transition-colors active:scale-95"
          >
            <Leaf size={12} /> 除草 ({plot.weedClicks})
          </button>
        )}
